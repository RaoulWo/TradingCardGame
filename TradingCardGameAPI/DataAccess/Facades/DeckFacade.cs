...
public class DeckFacade : IDeckFacade
{
    ...
    {
        string sqlStatement = "INSERT INTO deck (Id, fk_player_id, fk_collection_id) VALUES (@Id, @PlayerId, @CollectionId)";
        int rowsAffected = 0;
    ...
    public int Update(DeckEntity deck)
    {
        string sqlStatement = "UPDATE deck SET fk_player_id = @PlayerId, fk_collection_id = @CollectionId WHERE Id = @Id";
        int rowsAffected = 0;

        try
        {
            NpgsqlTransaction transaction = _unitOfWork.BeginTransaction();
            rowsAffected = _deckRepository.Update(deck, sqlStatement, transaction);
            _unitOfWork.CommitTransaction();
        }
        catch (Exception e)
        {
